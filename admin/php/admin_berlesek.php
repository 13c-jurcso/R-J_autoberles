<?php
    ...
    // Bérlés törlése
    if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['delete_berles'])){
        $berles_id = $_POST['berles_id'];

        $torles = $db->prepare("DELETE FROM berlesek WHERE berlesek.berles_id = ?");
        $torles->bind_param("i", $berles_id);
        var_dump($berles_id);
        if($torles->execute()){
            session_start();
    ...
    <div class="table-container">
        ...
        <?php
            $berlesek_listazasa_sql = "SELECT berlesek.berles_id, jarmuvek.gyarto, jarmuvek.tipus, felhasznalo.nev, berlesek.tol, berlesek.ig FROM berlesek 
                                       INNER JOIN jarmuvek ON berlesek.jarmu_id=jarmuvek.jarmu_id INNER JOIN felhasznalo ON felhasznalo.felhasznalo_nev=berlesek.felhasznalo ORDER BY berlesek.berles_id DESC;";
            $berlesek_listazasa = adatokLekerdezese($berlesek_listazasa_sql);
            ...
            if(is_array($berlesek_listazasa)){
                foreach ($berlesek_listazasa as $b) {
                    ...
                    echo '<td><form method="post">
                                ...
                                <button type="submit" name="delete_berles" class="torles_button">Törlés</button>
                          </form></td></tr>';
                }
            }
            else{
